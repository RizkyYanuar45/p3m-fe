...
const DokumenP3M = () => {
  ...
  useEffect(() => {
    const fetchAndGroupData = async () => {
      ...
      setError("");
      try {
        const response = await fetch(`${api}/dokumenprof/`);
        if (!response.ok) {
        ...
        <Container>
          <Row className="mb-5">
            <Col className="text-center">
              <h1 className="fw-bold">Dokumen P3M</h1>
              <p className="text-muted">
                Temukan semua dokumen penting terkait Penelitian,Publikasi dan
                Pengabdian Pada Masyarakat di sini.
              </p>
            </Col>
          </Row>
          <Row>
