...
const initialForm = { judul: '', deskripsi: '', tanggal: '', link: '', namaDosen: '' };
...
const CrudInformasiPengabdian = () => {
  ...
  return (
    <div>
      <h2>Kelola Informasi Pengabdian Kepada Masyarakat</h2>
      <form onSubmit={handleSubmit} style={{ background: '#f5f5f5', padding: 16, borderRadius: 8, marginBottom: 24 }}>
        ...
        <button type="submit" style={{ background: '#1976d2', color: '#fff', border: 'none', borderRadius: 4, padding: '8px 20px', fontWeight: 600, cursor: 'pointer' }}>{editIndex !== null ? 'Simpan Perubahan' : 'Tambah Informasi'}</button>
        {editIndex !== null && (
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td style={{ border: '1px solid #ddd', padding: 8 }}>{item.namaDosen}</td>
                  <td style={{ border: '1px solid #ddd', padding: 8 }}>{item.deskripsi}</td>
                  <td style={{ border: '1px solid #ddd', padding: 8 }}>{item.tanggal}</td>
                  <td style={{ border: '1px solid #ddd', padding: 8 }}>
                    <a href={item.link} target="_blank" rel="noopener noreferrer">Lihat</a>
                  </td>
                  <td style={{ border: '1px solid #ddd', padding: 8 }}>
