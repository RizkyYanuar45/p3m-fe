...
const CrudDokumenP3M = () => {
  ...
  const handleCategoryDelete = async (id) => {
    if (
      ...
      try {
        const response = await fetch(`${api}/catdokumenprof/delete/${id}`, {
          method: "DELETE",
          credentials: "include",
        });
        if (!response.ok) {
  ...
  const handleDocumentEdit = (doc) => {
    ...
    setDocForm({
      ...
      catdokumenprofId: doc.catdokumenprofId,
    });
  };
  ...
  const handleDocumentDelete = async (id) => {
    if (window.confirm("Yakin ingin menghapus dokumen ini?")) {
      try {
        const response = await fetch(`${api}/dokumenprof/delete/${id}`, {
          method: "DELETE",
          credentials: "include",
        });
        if (!response.ok) {
          ...
          <form onSubmit={handleDocumentSubmit} style={{ marginBottom: 16 }}>
            ...
            {editDocId && (
              <div style={{ marginTop: "10px" }}>
                ...
                <select
                  value={docForm.catdokumenprofId}
                  onChange={(e) =>
                    setDocForm({
                      ...docForm,
                      catdokumenprofId: Number(e.target.value),
                    })
                  }
                >
                  {categories.map((cat) => (
