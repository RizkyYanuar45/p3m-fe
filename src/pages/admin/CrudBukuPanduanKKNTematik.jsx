...
import { bukupanduankkntematik as initialbukupanduankkntematik } from '../../data/bukupanduankkntematik';

const CrudBukuPanduanKKNTematik = () => {
  const [bukupanduankkntematik, setbukupanduankkntematik] = useState(initialbukupanduankkntematik);
  ...
  return (
    <div>
      ...
      <form
        onSubmit={e => {
          ...
          if (editIndex !== null && editIndex > -1) {
            const updated = [...bukupanduankkntematik];
            updated[editIndex] = { ...form, id: updated[editIndex].id };
            setbukupanduankkntematik(updated);
            setEditIndex(null);
          } else {
            setbukupanduankkntematik([...bukupanduankkntematik, { ...form, id: Date.now() }]);
          }
      ...
      <table border="1" cellPadding="8" style={{ width: '100%' }}>
        ...
        <tbody>
          {bukupanduankkntematik.map((item, idx) => (
            <tr key={item.id}>
              <td>{idx + 1}</td>
              <td>{item.title}</td>
              ...
              <td>
                ...
                <button onClick={() => setbukupanduankkntematik(bukupanduankkntematik.filter((_, i) => i !== idx))}>Hapus</button>
              </td>
            </tr>
          ))}
        </tbody>
